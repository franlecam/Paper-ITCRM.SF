...
rubros     <- read_excel("Datos.xlsx", sheet = "rubros") #sheet = "rubros" tiene datos de santa fe, sheet = "rubros_total_arg" tiene datos de todas las provincias
...
participaciones <- rubros_pulido |>
  filter(Rubro %in% c("MOA", "MOI")) |> #("MOA", "MOI", "PP", "CyE")
...
participaciones <- rubros_pulido |>
  ...
  filter(Rubro %in% c("MOA", "MOI", "PP", "CyE")) |> # ("MOA", "MOI", "PP", "CyE")
  mutate(Año = as.integer(Año)) |>
...
library(tidyr)
library(dplyr)
write_xlsx(itcrm_wide, "itcrm_por_provincias.xlsx")

expo_mensual_prov <- rubros_arg %>%
  group_by(Año, CMES, DESCRIP_PCIA) %>%
  summarise(
    exportaciones_fob = sum(DOLARES_FOB, na.rm = TRUE),
    .groups = "drop"
  )

expo_wide <- expo_mensual_prov %>%
  pivot_wider(
    names_from = DESCRIP_PCIA,
    values_from = exportaciones_fob,
    values_fill = 0
  )

write_xlsx(itcrm_wide, "itcrm_por_provincias.xlsx")

expo_prov_rubro <- rubros_arg %>%
  filter(Rubro == "MOA") %>%  
  group_by(Año, CMES, DESCRIP_PCIA) %>%
  summarise(
    exportaciones_fob = sum(DOLARES_FOB, na.rm = TRUE),
